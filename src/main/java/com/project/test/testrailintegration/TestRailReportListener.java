...
//  }
  ...
  private void reportResult(ResultStatus status, ITestResult tr) {
    if (Config.isListenerEnabled()) {
      ...
      if (caseId > 0) {
        LogUtilities.logInfoMessage(format(
            "RailsIntegrator: save to memory result %s, for test case C%d", status.name(), caseId));
  ...
  public void addResultsToTestRun() {
    ...
    while (resultsIterator.hasNext()) {
      ...
      // Since both arrays are sorted, we may iterate through both of them simultaneously.
      ...
      if (projectCases[projectCasesIndex].id != currentTR.getCase_id()) {
        LogUtilities.logWarningMessage(
            format("There is no case with id=C%s in TestRail!", currentTR.getCase_id()));
        resultsIterator.remove();
        continue;
      }
      // Remove duplicates, update status to the worst, if at least test fail, fail the whole TC.
  ...
  private void updateTestCasesStatuses() {
    ...
    for (TestResult testResult : testResults.results) {
      ...
      // At this point we may grantee that projectCases[projectCasesIndex].id == testResult.case_id
